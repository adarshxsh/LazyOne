...
def resolve_dispute(dispute, winner=None, voting_jurors=None):
    ...
    with transaction.atomic():
        ...
        if dispute.escrow_status == 'held' and dispute.deposit_amount > 0:
            is_taker_winner = (winner == task.taken_by or winner == 'taker')
            is_poster_winner = (winner == task.posted_by or winner == 'poster')
            if (is_taker_winner and dispute.raised_by == task.taken_by) or (is_poster_winner and dispute.raised_by == task.posted_by):
