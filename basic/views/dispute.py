    ...
    if request.method == 'POST':
        ...
        with transaction.atomic():
            ...
            if hasattr(task, 'dispute'):
                ...
                dispute.response_deadline = deadline
                dispute.dispute_stage = 'counter_evidence'
                dispute.resolved_at = None
                dispute.resolution_outcome = None
                dispute.save()
            else:
            ...
            # Notify task poster / counterparty
            recipient = task.posted_by if request.user == task.taken_by else task.taken_by
            Notification.objects.create(
                recipient=recipient,
                message=f"{request.user.username} has raised a dispute for task '{task.title}'. Response deadline: {deadline.strftime('%Y-%m-%d %H:%M UTC')}.",
                link=reverse('dispute_detail', args=[dispute.id])
            )

        messages.success(request, f"Dispute raised successfully. {deposit_amount} points held as deposit bond. Counter-evidence deadline set to 72 hours.")
        return redirect('dispute_detail', dispute_id=dispute.id)
