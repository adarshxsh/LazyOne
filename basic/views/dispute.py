...
def resolve_dispute_settlement(panel, winning_choice=None):
    ...
    with transaction.atomic():
        ...
        # Handle deposit bond escrow
        if dispute.escrow_status == 'held' and dispute.deposit_amount > 0:
            if winning_choice == 'poster':
                if dispute.raised_by == task.posted_by:
                    dispute.refund_deposit(reason_description=f"Security deposit bond refunded for dispute #{dispute.id} settled in favor of poster")
                else:
                    dispute.forfeit_deposit(beneficiary=task.posted_by, reason_description=f"Security deposit bond forfeited for dispute #{dispute.id} settled in favor of poster")
            elif winning_choice == 'taker':
                if dispute.raised_by == task.taken_by:
                    dispute.refund_deposit(reason_description=f"Security deposit bond refunded for dispute #{dispute.id} settled in favor of taker")
                else:
                    dispute.forfeit_deposit(beneficiary=task.taken_by, reason_description=f"Security deposit bond forfeited for dispute #{dispute.id} settled in favor of taker")
            else:
                dispute.refund_deposit(reason_description=f"Security deposit bond refunded for dispute #{dispute.id} split settlement")
