    ...
    with transaction.atomic():
        if resolution_type == 'award_taker':
            ...
            if dispute.escrow_status == 'held' and dispute.deposit_amount > 0:
                if dispute.raised_by == taker:
                    dispute.refund_deposit(reason_description=f"Security deposit bond refunded for successful dispute on task: '{task.title}'")
                else:
                    dispute.forfeit_deposit(beneficiary=taker, reason_description=f"Security deposit bond forfeited for unsuccessful dispute on task: '{task.title}'")
            else:
                dispute.save()
            ...
            if dispute.escrow_status == 'held' and dispute.deposit_amount > 0:
                if dispute.raised_by == poster:
                    ...
                    dispute.forfeit_deposit(beneficiary=poster, reason_description=f"Security deposit bond forfeited for unsuccessful dispute on task: '{task.title}'")
            else:
                dispute.save()
            ...
            dispute.status = 'resolved'
            if dispute.escrow_status == 'held' and dispute.deposit_amount > 0:
                dispute.refund_deposit(reason_description=f"Security deposit bond refunded upon dispute dismissal on task: '{task.title}'")
            else:
                dispute.save()
