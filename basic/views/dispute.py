...
def dispute_detail_view(request, dispute_id):
    ...
    task = dispute.task
    is_participant = request.user in [task.posted_by, task.taken_by] or request.user == dispute.raised_by
    if not is_participant and not request.user.is_staff and dispute.status not in ['open', 'voting']:
        messages.error(request, "You are not authorized to view this dispute.")
        return redirect('home')

    user_vote = DisputeVote.objects.filter(dispute=dispute, voter=request.user).first()
    can_vote = (not is_participant) and (dispute.status in ['open', 'voting']) and (user_vote is None)
...
def raise_dispute(request, task_id):
    ...
    if request.method == 'POST':
        ...
        deposit_amount = min(task.reward, task.deposit_bond_amount)
        taker_profile = request.user.userprofile
        poster_profile = task.posted_by.userprofile

        if taker_profile.rewards < deposit_amount:
            messages.error(
                request,
                f"Insufficient reward points balance. You need at least {deposit_amount} points as a deposit bond to raise a dispute, but you only have {taker_profile.rewards} points."
            )
            return redirect('my_tasks')
        ...
        if poster_profile.rewards < deposit_amount:
            messages.error(
                request,
                f"Task poster has insufficient reward points balance ({poster_profile.rewards} points) to match required deposit bond ({deposit_amount} points). Dispute cannot be opened."
            )
            return redirect('my_tasks')

        with transaction.atomic():
            taker_profile.rewards -= deposit_amount
            taker_profile.save()
            ...
            RewardLedger.objects.create(
                ...
                description=f"Poster matching deposit bond held for dispute on task: '{task.title}'"
            )
            ...
            Notification.objects.create(
                ...
                message=f"{request.user.username} has raised a dispute for your task: '{task.title}'. Matching deposit bond of {deposit_amount} points held.",
                link=reverse('dispute_detail', args=[dispute.id])
            )
        messages.success(request, f"Dispute raised successfully. Symmetrical deposit bond of {deposit_amount} points held for both worker and poster.")
...
def withdraw_dispute(request, dispute_id):
    ...
    with transaction.atomic():
        dispute.refund_deposit(
            reason_description=f"Deposit bond refunded for withdrawn dispute on task: '{task.title}'"
        )
    ...
    messages.success(request, f"You have successfully withdrawn the dispute for '{task.title}'. All deposit bonds have been refunded.")
    return redirect('my_tasks')
...
def vote_dispute(request, dispute_id):
    ...
    if request.user == task.posted_by or request.user == task.taken_by or request.user == dispute.raised_by:
        messages.error(request, "Task participants are not eligible to vote as jurors.")
        return redirect('dispute_detail', dispute_id=dispute.id)
    ...
    if juror_profile.rewards < stake_amount:
        messages.error(request, f"Insufficient reward points balance. You need at least {stake_amount} points as a juror stake commitment.")
        return redirect('dispute_detail', dispute_id=dispute.id)
