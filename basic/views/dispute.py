...
def select_jury_for_dispute(dispute):
    ...
    # Candidate users: active users excluding direct participants and friends
    ...
    Jury.objects.filter(dispute=dispute).delete()

    for juror in selected_jurors:
        Jury.objects.create(dispute=dispute, juror=juror)
...
def dispute_detail_view(request, dispute_id):
    ...
    is_juror = Jury.objects.filter(dispute=dispute, juror=request.user).exists()

    if not is_disputant and not is_juror and not request.user.is_staff:
        messages.error(request, "You are not authorized to view this dispute.")
        return redirect('home')

    poster_votes = JuryVote.objects.filter(dispute=dispute, vote__in=['poster', 'poster_wins']).count()
    taker_votes = JuryVote.objects.filter(dispute=dispute, vote__in=['taker', 'taker_wins']).count()
...
def vote_dispute(request, dispute_id):
    ...
    if not Jury.objects.filter(dispute=dispute, juror=request.user).exists():
    ...
    if vote_choice not in ['poster', 'taker', 'poster_wins', 'taker_wins']:
        messages.error(request, "Invalid vote choice.")
        return redirect('dispute_detail', dispute_id=dispute.id)

    normalized_vote = 'poster' if vote_choice in ['poster', 'poster_wins'] else 'taker'

    with transaction.atomic():
        JuryVote.objects.create(dispute=dispute, juror=request.user, vote=normalized_vote)

        poster_votes = JuryVote.objects.filter(dispute=dispute, vote__in=['poster', 'poster_wins']).count()
        taker_votes = JuryVote.objects.filter(dispute=dispute, vote__in=['taker', 'taker_wins']).count()

        total_jurors = Jury.objects.filter(dispute=dispute).count()
        majority_needed = (total_jurors // 2) + 1 if total_jurors > 0 else 2
