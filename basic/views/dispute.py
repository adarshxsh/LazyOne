...
def withdraw_dispute(request, dispute_id):
    ...
    with transaction.atomic():
        ...
        initiator = request.user if request.user in (task.posted_by, task.taken_by) else dispute.raised_by
        recipient = task.taken_by if initiator == task.posted_by else task.posted_by
        if recipient:
            Notification.objects.create(
                recipient=recipient,
