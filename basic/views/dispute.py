...
def raise_dispute(request, task_id):
    ...
    if request.user not in (task.posted_by, task.taken_by) or task.status != 'in_progress':
        messages.error(request, "You can only raise a dispute for an in-progress task that you posted or took.")
    ...
    if request.method == 'POST':
        ...
        if counterparty:
            Notification.objects.create(
                ...
                message=f"{request.user.username} has raised a dispute for '{task.title}'.",
...
def withdraw_dispute(request, dispute_id):
    ...
    if dispute.raised_by != request.user:
        messages.error(request, "Only the user who raised the dispute can withdraw it.")
        return redirect('my_tasks')
    task = dispute.task
