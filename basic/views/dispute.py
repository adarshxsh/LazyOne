        ...
        with transaction.atomic():
            ...
            if dispute.escrow_status == 'held' and dispute.deposit_amount > 0:
                if winner == dispute.raised_by:
                    dispute.refund_deposit(
                        reason_description=f"Security deposit bond refunded upon automated dispute settlement for task: '{task.title}'"
                    )
                else:
                    dispute.forfeit_deposit(
                        reason_description=f"Security deposit bond forfeited upon automated dispute settlement for task: '{task.title}'"
                    )
