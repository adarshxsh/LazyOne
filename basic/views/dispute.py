...
from ..models import Dispute, DisputeEvidence, DisputeJuror, Task, Notification, RewardLedger, Conversation
...
def dispute_detail_view(request, dispute_id):
    ...
    if not main_chat:
        ...
        if task.posted_by:
            main_chat.participants.add(task.posted_by)
        if task.taken_by:
            main_chat.participants.add(task.taken_by)

    evidence_list = dispute.evidence.all()
    can_upload = dispute.status == 'open' and (is_party or request.user.is_staff)
    ...
    context = {
        ...
        'main_chat': main_chat,
        'jurors': dispute.jurors.select_related('user').all(),
    }
