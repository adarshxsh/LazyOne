...
from ..models import Conversation, Message, Notification, Jury
...
def chat_view(request, conversation_id):
    ...
    if request.user not in conversation.participants.all():
        if conversation.task and hasattr(conversation.task, 'dispute'):
            ...
            if dispute.status == 'open' and Jury.objects.filter(dispute=dispute, juror=request.user).exists():
                is_read_only = True
                logger.info("Step 2: User is an assigned peer juror for this open dispute. Access granted (read-only).")
            else:
                logger.warning("Step 2: User is not an assigned juror or dispute is not open. Redirecting to home.")
