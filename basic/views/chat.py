...
def chat_view(request, conversation_id):
    ...
    is_dispute_juror = False

    if not is_participant and conversation.task and conversation.task.status == 'disputed':
        if hasattr(conversation.task, 'dispute') and hasattr(conversation.task.dispute, 'jury'):
            if request.user in conversation.task.dispute.jury.jurors.all():
                is_dispute_juror = True

    if not is_participant and not is_dispute_juror:
        logger.warning("Step 2: User is neither participant nor active dispute juror. Redirecting to home.")
    ...
    context = {
        ...
        'is_dispute_juror': is_dispute_juror,
    }
