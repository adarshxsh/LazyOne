...
def chat_view(request, conversation_id):
    ...
    if not is_participant:
        is_disputed = (
            conversation.task is not None and 
            conversation.task.status == 'disputed'
        )
        if is_disputed:
            is_read_only = True
            logger.info("Step 2: User is not a participant, but task is disputed. Granting read-only access.")
        else:
