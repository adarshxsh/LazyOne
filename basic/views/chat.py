...
def chat_view(request, conversation_id):
    ...
    if conversation.task and hasattr(conversation.task, 'dispute'):
        ...
        is_assigned_juror = JuryAssignment.objects.filter(dispute=conversation.task.dispute, juror=request.user).exists()
    ...
    if not is_participant and not is_assigned_juror and not request.user.is_staff:
        ...
        return redirect('home')
    logger.info("Step 2: User authorized to view chat.")
