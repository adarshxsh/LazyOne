...
def take_task(request, task_id):
    ...
    if task.posted_by == request.user:
        ...
        if taker_profile.rewards < collateral:
            messages.error(request, f"You need at least {collateral} collateral points to claim this task, but you only have {taker_profile.rewards} points.")
        ...
        with transaction.atomic():
            ...
            messages.success(request, f"Task has been assigned to you. {collateral} points collateral locked.")
    return redirect('my_tasks')
...
def complete_task(request, task_id):
    ...
    with transaction.atomic():
        ...
        messages.success(request, f"Task marked as complete! {task.reward} points transferred to {task.taken_by.username} and {collateral} points collateral released.")
    return redirect('my_tasks')
...
def accept_cancellation(request, task_id):
    ...
    with transaction.atomic():
        ...
        messages.success(request, f"You have accepted the cancellation. Your {collateral} points collateral has been refunded.")
    return redirect('my_tasks')
...
def abandon_task(request, task_id):
    ...
    with transaction.atomic():
        ...
        RewardLedger.objects.create(
            user=task.posted_by, task=task, amount=collateral,
            transaction_type='collateral_slash', description=f"Received slashed collateral from {request.user.username} for task: '{task.title}'"
        )
        RewardLedger.objects.create(
            user=request.user, task=task, amount=-collateral,
            transaction_type='collateral_slash', description=f"Collateral slashed for abandoning task: '{task.title}'"
        )
        ...
        Notification.objects.create(
            recipient=task.posted_by,
            message=f"{request.user.username} has abandoned your task: '{task.title}'. Slashed collateral of {collateral} points was credited to you.",
            link=reverse('my_tasks')
        )
        messages.success(request, f"You have abandoned the task. Your {collateral} points collateral has been slashed.")
    return redirect('my_tasks')
