...
def complete_task(request, task_id):
    ...
    if task.status == 'disputed':
        messages.error(request, "This task is currently in dispute. Unilateral completion is prohibited while active jury arbitration is ongoing.")
        return redirect('my_tasks')
    if task.status != 'in_progress':
        messages.error(request, "Task is not currently in progress.")
        return redirect('my_tasks')
    ...
    with transaction.atomic():
        ...
        if hasattr(task, 'dispute') and task.dispute.status == 'open':
            ...
            for assignment in task.dispute.assignments.filter(is_staked=True):
                ...
                RewardLedger.objects.create(
                    ...
                    description=f"Refund of staked points due to task completion on task: '{task.title}'"
                )
