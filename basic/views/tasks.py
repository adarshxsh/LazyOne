...
def abandon_task(request, task_id):
    ...
    with transaction.atomic():
        taker_profile, _ = UserProfile.objects.select_for_update().get_or_create(user=request.user)
        penalty = task.abandonment_penalty

        # Balance guardrail: rewards balance must never drop below 0
        taker_profile.rewards = max(0, taker_profile.rewards - penalty)
        taker_profile.tasks_abandoned = taker_profile.tasks_abandoned + 1
        if hasattr(taker_profile, 'reputation_score'):
            taker_profile.reputation_score = max(0, taker_profile.reputation_score - 10)
        taker_profile.save()
        ...
        RewardLedger.objects.create(
            ...
            task=task,
            amount=-penalty,
            transaction_type='task_abandon_penalty',
            description=f"Task Abandonment Penalty for task: '{task.title}'"
        )

        # Clean up dispute if present
        if hasattr(task, 'dispute'):
            task.dispute.delete()

        # Reset task status to available
        task.status = 'available'
        task.taken_by = None
        task.cancellation_requested = False
        task.save()
        ...
        messages.success(request, f"You have abandoned the task. A penalty of {penalty} points was deducted from your reward balance.")
    return redirect('my_tasks')
