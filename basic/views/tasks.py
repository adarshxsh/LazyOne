...
def take_task(request, task_id):
    ...
    if task.posted_by == request.user:
        ...
        collateral = task.collateral_required
        user_profile = request.user.userprofile
        if user_profile.rewards < collateral:
            messages.error(request, f"You need at least {collateral} points balance for collateral to take this task.")
            return redirect('my_tasks')

        with transaction.atomic():
            user_profile.rewards -= collateral
            user_profile.save()
            ...
            messages.success(request, f"Task has been assigned to you. {collateral} points locked as collateral.")
    return redirect('my_tasks')
...
def complete_task(request, task_id):
    ...
    with transaction.atomic():
        ...
        messages.success(request, f"Task marked as complete! {task.reward} reward points and {collateral} collateral points transferred to {task.taken_by.username}.")
    return redirect('my_tasks')
...
def abandon_task(request, task_id):
    ...
    with transaction.atomic():
        ...
        RewardLedger.objects.create(
            ...
            transaction_type='collateral_slash', description=f"Slashed collateral compensation for abandoned task: '{task.title}'"
        )
        ...
        Notification.objects.create(
            recipient=task.posted_by,
            message=f"{request.user.username} has abandoned your task: '{task.title}'. Slashed collateral of {collateral} points credited to your balance.",
            link=reverse('my_tasks')
        )
        messages.success(request, "You have abandoned the task. Your collateral has been slashed.")
    return redirect('my_tasks')
