...
def abandon_task(request, task_id):
    ...
    penalty_amount = getattr(settings, 'TASK_ABANDONMENT_PENALTY', 50)
    with transaction.atomic():
        user_profile = request.user.userprofile
        user_profile.rewards -= penalty_amount
        user_profile.save()
        ...
        messages.success(request, f"You have abandoned the task. A penalty of {penalty_amount} points has been deducted.")
    return redirect('my_tasks')
