...
def accept_cancellation(request, task_id):
    task = Task.objects.filter(id=task_id, taken_by=request.user, status='in_progress', cancellation_requested=True).first()
    if not task:
        messages.error(request, "Cannot accept cancellation: task is not in progress or cancellation was not requested.")
        return redirect('my_tasks')
