...
def complete_task(request, task_id):
    ...
    if task.status == 'disputed':
        messages.error(request, "This task is currently under dispute and cannot be completed unilaterally.")
        return redirect('my_tasks')
    if task.status != 'in_progress':
        messages.error(request, "Only tasks in progress can be marked as complete.")
        return redirect('my_tasks')
