...
def complete_task(request, task_id):
    ...
    if task.status == 'disputed':
        messages.error(request, "Tasks under dispute cannot be completed until the dispute is resolved or withdrawn.")
        return redirect('my_tasks')
    if task.status != 'in_progress':
        messages.error(request, "Task cannot be completed in its current status.")
        return redirect('my_tasks')
