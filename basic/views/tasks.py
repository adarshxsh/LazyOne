...
def complete_task(request, task_id):
    ...
    if task.status == 'disputed':
        messages.error(request, "Cannot complete a task that is under dispute. Active disputes must be resolved through jury consensus or withdrawal.")
        return redirect('my_tasks')
    if task.status != 'in_progress':
        messages.error(request, "This task cannot be marked as complete.")
        return redirect('my_tasks')
