...
class Dispute(models.Model):
    ...
    resolution_rationale = models.TextField(null=True, blank=True)
    ...
    def is_appealable(self):
        ...
        if hasattr(self, 'appeal'):
            return False
        window_days = getattr(settings, 'DISPUTE_APPEAL_WINDOW_DAYS', 7)
        return timezone.now() <= self.resolved_at + timedelta(days=window_days)
...
class DisputeAppeal(models.Model):
    ...
    appealed_by = models.ForeignKey(User, on_delete=models.CASCADE, related_name='dispute_appeals')
    reason = models.TextField()
    status = models.CharField(max_length=20, choices=STATUS_CHOICES, default='pending')
    created_at = models.DateTimeField(auto_now_add=True)

    # Senior staff review tracking
    ...
    def __str__(self):
        return f"Appeal for dispute #{self.dispute.id} by {self.appealed_by.username}"
