...
from .views.dispute import sync_dispute_to_firestore
...
class DisputeFirestoreSyncTestCase(TestCase):
    ...
    def test_withdraw_dispute_syncs_and_deletes_firestore_doc(self, mock_init_fb, mock_fb_admin, mock_firestore):
        dispute = Dispute.objects.create(task=self.task, raised_by=self.claimer, reason='Initial reason', deposit_amount=50)
