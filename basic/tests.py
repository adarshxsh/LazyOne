...
class TaskCancellationTests(TestCase):
    ...
    def test_accept_cancellation_resets_reward_and_sets_cancelled_status(self):
        # 1. Taker claims task
        ...
        # Cancelled task is not in available task list
        available_tasks = Task.objects.filter(status='available')
        self.assertNotIn(self.task, available_tasks)

        # Other users cannot take or complete the cancelled task (prevents double reward exploits)
