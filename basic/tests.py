...
class DisputeStatusGuardsTestCase(TestCase):
    ...
    def test_withdraw_dispute_success(self):
        """withdraw_dispute reverts task status from 'disputed' to 'in_progress' and sets dispute status to resolved."""
        self.task.status = 'disputed'
        self.task.save()
        dispute = Dispute.objects.create(task=self.task, raised_by=self.taker1, reason='Valid dispute', status='open', deposit_amount=50, escrow_status='held')
    ...
    def test_raise_dispute_cleans_up_resolved_dispute_for_new_taker(self):
        """raise_dispute updates resolved dispute for new task taker."""
