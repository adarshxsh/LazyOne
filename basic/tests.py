...
class DisputeDepositBondTests(TestCase):
    ...
    def test_complete_disputed_task_refunds_deposit(self):
        ...
        # Poster attempts to mark disputed task as completed -> rejected by state guard
        ...
        # Taker withdraws dispute -> deposit refunded (40 + 60 = 100), task status returns to 'in_progress'
        dispute = Dispute.objects.get(task=self.task)
        self.client.login(username='taker', password='password123')
        ...
        # Taker balance: 100 (after refund) + 300 (task reward) = 400
        self.taker_profile.refresh_from_db()
...
class TaskStateGuardTestCase(TestCase):
    ...
    def test_withdraw_dispute_success_future_deadline(self):
        self.task.status = 'disputed'
        self.task.save()
        dispute = Dispute.objects.create(task=self.task, raised_by=self.taker, reason='Issue with task', deposit_amount=50, escrow_status='held')
