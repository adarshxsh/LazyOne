...
class ProRataDisputeSettlementTests(TestCase):
    ...
    def test_resolve_dispute_math_floor_odd_reward(self):
        # Task reward 95, 50% split -> taker gets floor(47.5)=47, poster gets 95-47=48
        odd_task = Task.objects.create(
            title="Odd Reward Task",
            description="Odd reward description",
            reward=95,
            posted_by=self.poster,
            taken_by=self.taker,
            status='disputed',
        ...
        response = self.client.post(
            reverse('resolve_dispute', args=[odd_dispute.id]),
            {
                'percent': '50',
                'deposit_action': 'refund',
                'settlement_notes': '50/50 compromise'
            }
        )

        payout_ledger = RewardLedger.objects.get(
            user=self.taker, task=odd_task, transaction_type='dispute_partial_payout'
        )
        refund_ledger = RewardLedger.objects.get(
            user=self.poster, task=odd_task, transaction_type='dispute_partial_refund'
        )

        self.assertEqual(payout_ledger.amount, 47)
        self.assertEqual(refund_ledger.amount, 48)
        self.assertEqual(payout_ledger.amount + refund_ledger.amount, 95)
    ...
    def test_resolve_dispute_already_resolved(self):
