...
class DisputeHandlingTestCase(TestCase):
    ...
    def test_withdraw_dispute_sets_status_withdrawn_and_preserves_row(self):
        dispute = Dispute.objects.create(task=self.task, raised_by=self.taker, reason='Dispute reason', status='open', deposit_amount=50, escrow_status='held')
        self.task.status = 'disputed'
        self.task.save()
    ...
    def test_poster_can_withdraw_own_dispute(self):
        dispute = Dispute.objects.create(task=self.task, raised_by=self.poster, reason='Poster dispute', status='open', deposit_amount=50, escrow_status='held')
        self.task.status = 'disputed'
        self.task.save()
